...
<_halLinkInit>
	...
	push ar0, gr0;
	push ar1, gr1;
	//вычисление номера канала и зануление callback'a
	// CP0TR - 0
	// CP0RC - 1
	// CP1TR - 2
	// CP1RC - 3
	// CP2TR - 4
	// CP2RC - 5
	gr0 = [--ar5]			with gr7 = false;	//номер порта 0..2
	gr1 = [--ar5]			with gr0 <<= 1;		//если направление равно 0 то порт передающий, если 1 то порт принимающий
	ar0 = cpUserCallback	with gr0 += gr1;
...
<EndShift>
	...
	//Запись вектора прерывания. Вектора прерываний начинаются с адреса 20h и каждый вектор занимает по 4 длинных слова
	//Прерывания линков начинаются с 24 номера
	ar0 = 20h + 24 * 8h 	with gr0 <<= 3;
	ar0 += gr0;
	ar1 = cpInterrupts	with gr1 = gr0;
	ar5 = ar1 + gr1;
	ar1,gr1 = [ar5++];
	[ar0++] = ar1, gr1;
	ar1,gr1 = [ar5++];
	...
	[4000_040Ah] = gr7;		//enable link interrupt*/
