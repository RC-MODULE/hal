...
typedef int(*DmaCallback2)();
	...
	*/
	...
	/**
		* \brief Функция выполняет инициализацию ПДП для работы с callBack функцией и должна быть вызвана 1 раз перед запуском ПДП 
	*/
	int  halOpenDMA();
	/**
		* \brief Функция выполняет инициализацию ПДП для работы без callBack функции и должна быть вызвана 1 раз перед запуском ПДП.
