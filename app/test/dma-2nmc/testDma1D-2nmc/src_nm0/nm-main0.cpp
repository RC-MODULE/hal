...
void*  halCopyDMA   (const void* src,  void* dst,  unsigned size32){
	...
	halDmaStartC((void*)src, dst, size32);
...
int main()
{
	...
	if (0) for(int i=0; i<1111600;i++){
		halCopyDMA(src[0],dst[0],128);
		//halSync(0,1);
		//halSync(0,1);
		halLed(i>>8);
		//halSync(0,0);
	...
	for(int k=1; k<1000; k++){
		...
		for(int i=0;i<4 ;i++){
			for(int j=0; j<4; j++){
				//halLed4Lo(j*k);
				for(int srcDisp=0; srcDisp<16; srcDisp+=16){
					//halLedOn(6);
					for(int dstDisp=0; dstDisp<16; dstDisp+=16){
						//halLed4Lo(dstDisp);
