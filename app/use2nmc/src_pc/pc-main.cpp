...
int main(){
	...
	sync        =ubcSync(0x8086,0);			// receive repost proc1-proc0 sync // 101
	sync        =ubcSync(0x8086,1);			// receive repost proc0-proc1 sync // 100
	...
	size        =ubcSync(201,1);	// send shared addr of proc-0
	srcBrdAddr1 =ubcSync(202,1);				// get shared addr of proc-1
	dstBrdAddr1 =ubcSync(203,1);				// get shared addr of proc-1
	
	ubcReadMemBlock (srcHostArray,srcBrdAddr1,size,1); // read data from board 1
