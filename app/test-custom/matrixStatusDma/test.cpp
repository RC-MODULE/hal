...
int main(){ 
	...
	for (int srcBankIndx = 0; srcBankIndx < 4; srcBankIndx++) {
		for (int dstBankIndx = 0; dstBankIndx < 4; dstBankIndx++) {
			...
			for(int offset = 0; offset < 100; offset+=16){
				printf("Current offset %d\n",offset);
				for(int height = 0; height < 100; height++){
					for(int width = 0; width < 100; width += 16){
						SetArr(dst,Heap_size,0xcccccccc);
						SetArr(arr_ref,Heap_size,0xcccccccc);
						call_counter++;
						halInitStatusMatrixDMA(src + offset,width,height,width,dst,width);
						int count = 0;
						while(halStatusDMA()){
							if(count++ > 1000){
